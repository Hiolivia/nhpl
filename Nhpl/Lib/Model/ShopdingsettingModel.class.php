...
class ShopdingsettingModel extends CommonModel{
    protected $pk   = 'shop_id';
    protected $tableName =  'shop_ding_setting';
	...
	public function get_time($shop_id)
	{
		...
		$tem = array();
		foreach($cfg as $k => $v){
			if($k>=$setting['start_time'] && $k<= $setting['end_time']){
				$tem[$k] = $v;
			}
		}
		 return $tem;
	...
	public function get_is_room($shop_id,$data)
	{
		...
		$yuyue = D('Shopdingyuyue')->where($arr)->select();
		$tem = $t = array();
