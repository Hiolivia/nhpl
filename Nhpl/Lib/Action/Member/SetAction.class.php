...
class SetAction extends CommonAction{
    ...
    public function nickname(){
        if ($this->isPost()) {
			...
			$ext_account = $this->_post('ext_account', 'htmlspecialchars');
			$ext_mobile = $this->_post('ext_mobile', 'htmlspecialchars');
            $data = array('user_id' => $this->uid,'nickname' => $nickname,'ext_account' => $ext_account,'ext_mobile' => $ext_mobile);
            if (false !== D('Users')->save($data)) {
                $this->baoSuccess('设置成功！', U('set/nickname'));
            }
            $this->baoError('设置失败');
        } else {
            $this->display();
