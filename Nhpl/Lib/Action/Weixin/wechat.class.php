...
class WeChatClient {
	...
	{
		if($shop = (int)$shop){
			$shop_list = D('Shopdetails')->find($shop);
		}else{
			$shop_list = $shop;
		}
		$access_token = $this->getShopToken($shop_list);
    ...
    public function sendTempMsg($openid, $template_id, $url, $params)
    {
        ...
        $data['data']['first'] = array('value'=>urlencode($title), 'color'=>'#173177');        
        $index = 0;
        foreach((array)$params['items'] as $v){
            $index ++;
            $data['keynote'.$index] = array('value'=>urlencode($v), 'color'=>'#173177');
        }
        $remark = $params['remark'] ? $params['remark'] : '点击【详情】查看详细信息';
        $data['data']['remark'] = array('value'=>urlencode($remark), 'color'=>'#173177');
