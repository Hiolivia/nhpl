...
class MsgAction extends CommonAction {
	...
	public function detail($msg_id){
		...
		if(empty($detail)){
			$this->error('没有该信息或该信息已被删除！', U('fenzhan/index/main'));
		}
		//如果收件人不等于当前ID就。可是同意是发件人怎么样处理呢？
		if($detail['city_id'] != $this->city_id){
			$this->error('不能阅读其他分站的信息！', U('fenzhan/index/main'));
		}
		
		$this->assign('user', D('Users')->find($detail['send_id'])); // 赋值数据集www.hatudou.com  二开开发qq  120585022
