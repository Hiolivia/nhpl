...
class AreaAction extends CommonAction {
    ...
    public function index() {
		...
		$city_ids = D('City')->where($mapss)->order(array('city_id' => 'desc'))->select();//查询所在城市的商家
		foreach ($city_ids as $val) {
			$cityids[$val['city_id']] = $val['city_id'];//对比shop_id
		}
		$maps['city_id']  = array('in',$cityids);//取得当前商家ID，给下面的maps查询
        ...
        $count = $Area->where($maps)->count(); // 查询满足要求的总记录数 
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            $this->baoError('操作失败！');
             ...
             $this->assign('citys',$citys = D('City')->where(array('closed' => 0,'city_id'=>$this->city_id))->select());//这里应该查询fetchAll不过有缓存会错
            $this->display();
        }
    }
    ...
    public function edit($area_id = 0) {
        ...
        if ($area_id = (int) $area_id) {
            ...
            if ($this->isPost()) {
                ...
                $this->assign('citys',$citys = D('City')->where(array('closed' => 0,'city_id'=>$this->city_id))->select());//这里应该查询fetchAll不过有缓存会
                $this->display();
            }
        } else {
            $this->baoError('请选择要编辑的区域管理');
