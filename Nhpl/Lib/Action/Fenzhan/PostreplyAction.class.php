...
class PostreplyAction extends CommonAction{
	...
	//调用数据开始        
            $map_post = array('city_id' => $this->city_id);//查询城市ID为当前登录账户的ID
                $list_post = D('Post')->where($map_post)->select();//查询所在城市的商家
                foreach ($list_post as $val) {
                        $post_ids[$val['post_id']] = $val['post_id'];//对比shop_id
                }
                //调用数据结束        
				
				
       $map['post_id'] = array('IN',$post_ids);
