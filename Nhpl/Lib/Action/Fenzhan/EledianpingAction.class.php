...
class EledianpingAction extends CommonAction {
    ...
    public function index() {
        ...
        $count = $Eledianping->where($maps)->count(); // 查询满足要求的总记录数 
        $Page = new Page($count, 25); // 实例化分页类 传入总记录数和每页显示的记录数
        $show = $Page->show(); // 分页显示输出
        $list = $Eledianping->where($maps)->order(array('order_id' => 'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                ...
                $this->baoSuccess('添加成功', U('eledianping/index'));
            }
            $this->baoError('操作失败！');
        } else {
            $this->display();
    ...
    private function createCheck() {
        ...
        if (empty($data['order_id'])) {
            $this->baoError('外卖订单不能为空');
        }
        if (!$order = D('Eleorder')->find($data['order_id'])) {
            $this->baoError('外卖订单不存在');
        }
        ...
        if ($order_id = (int) $order_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    ...
                    $this->baoSuccess('操作成功', U('eledianping/index'));
                }
                $this->baoError('操作失败');
            } else {
    ...
    private function editCheck() {
	 ...
	 public function delete($order_id = 0) {
        if (is_numeric($order_id) && ($order_id = (int) $order_id)) {
            ...
            $this->baoSuccess('删除成功！', U('eledianping/index'));
        } else {
