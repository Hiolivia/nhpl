...
class ShopdianpingAction extends CommonAction {
    ...
    public function index() {
		...
		$map = array('city_id' => $this->city_id);//查询城市ID为当前登录账户的ID
		$shop_city = D('Shop')->where($map)->order(array('shop_id' => 'desc'))->select();//查询所在城市的商家
        ...
        $count = $Shopdianping->where($maps)->count(); // 查询满足要求的总记录数 
        $Page = new Page($count, 25); // 实例化分页类 传入总记录数和每页显示的记录数
        $show = $Page->show(); // 分页显示输出
        $list = $Shopdianping->where($maps)->order(array('dianping_id' => 'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
