...
class GoldeggAction extends CommonAction {
    private $create_fields = array('id','predict_num','shop_id','keyword','photo','title','use_tips','stime','ltime','info','aginfo','end_tips','end_photo','fist','fistnums','fistlucknums','second','secondnums','secondlucknums','third','hirdnums','hirdlucknums','oinnum','max_num','parssword','four','fournums','ourlucknums','five','fivenums','fivelucknums','six','ixnums','sixlucknums','daynums','member_condtion','dateline','follower_condtion');
	 ...
	 public function index($page=1)
    {
		...
		foreach ($list as $k => $val) {
            $url = U('weixin/goldegg/show', array('goldegg_id' => $val['id']));
            $url = __HOST__ . $url;
            $tooken = 'goldegg_' . $val['id'];
	...
	public function create()
	{
		...
		if ($this->isPost()) {
			...
			if ($id = $obj->add($data)) {
				 $this->baoSuccess('添加成功', U('goldegg/index'));
            }else{
				$this->baoError('添加失败！');
			}	
		}else{
      ...
      public function edit($id = 0) {
        if ($id = (int) $id) {
			...
			/*	$objk = D('Shop_weixin_keyword');
				...
				$keyword['url'] = $data[''];
				$keyword['photo'] = $data['end_photo'];
				if(false !== $objk->save($keyword)){
					$this->baoError('添加关键字失败！');
				}	
			}*/				
                if (false !== $obj->save($data)) {
                    $this->baoSuccess('修改成功', U('goldegg/index'));
                }
                $this->baoError('修改失败');
            } else {
	...
	public function delete($id=null)
    {
        ...
        if($id = (int)$id){
			if(!$detail = $obj->find($id)){
				...
				$this->baoSuccess('删除成功！',U('goldegg/index'));
			}
        }
    }  
	...
	public function sn($id = 0) {
        if ($id = (int) $id) {
            $obj = D('Weixin_goldeggsn');
			$obje = D('Weixin_goldegg');
	...
	public function snedit($sn_id=null)
    {
        ...
        if($sn_id = (int)$sn_id){
			if(!$detail = $obj->find($sn_id)){
				...
				$data['sn_id'] = $sn_id;
                if($obj->save($data)){
					$this->baoSuccess('修改成功！',U('goldegg/sn',array('id'=>$detail['egg_id'])));
                }
            }
        }
    }  
	public function sndelete($sn_id=null)
    {
        ...
        if($sn_id = (int)$sn_id){
			if(!$detail = $obj->find($sn_id)){
				...
				$this->baoSuccess('删除成功！',U('goldegg/sn',array('id'=>$detail['egg_id'])));
			}
        }
    }  
