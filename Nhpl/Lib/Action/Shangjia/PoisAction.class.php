...
class  PoisAction extends CommonAction{
    ...
    public function index() {
        ...
        $count = $Pois->where($map)->count(); // 查询满足要求的总记录数 
    ...
    public function buy($word_id = 0) {
		...
		if($this->MEMBER['gold'] < $word[price]){
			...
			if(empty($list)){
				...
				$this->assign('word', $word); // 赋值数据集www.hatudou.com  二开开发qq  120585022
				$this->assign('list', $list); // 赋值数据集www.hatudou.com  二开开发qq  120585022
				$this->display();
			}
		}
    ...
    public function pay($word_id = 0) {
		...
		if($this->MEMBER['gold'] < $word[price]){
			...
			if(empty($pois)){
				...
				$eprice = - $word['price'];	//反算价格
				$estick =  $word['text'];
				$etime =  strtotime(date('Y',time()) + 1 . '-' . date('m-d H:i:s',time()));

				if(D('Users')->addGold($this->uid, $eprice ,'购买黄页关键词包年服务，关键词：'.$word['text'])){
	...
	public function raise($pois_id = 0){
		...
		if($this->MEMBER['gold'] <= 0){
			  $this->baoError('账户余额不足！');
		}else{
			if(D('Users')->addGold($this->uid, -1,'提升黄页权重1点')){
	...
	public function renew($word_id = 0){
		...
		if($this->MEMBER['gold'] < $price){
			  $this->baoError('账户余额不足，需要'.$price.'金块进行续费！');
		}else{
			$etime = strtotime(date('Y',$time) + 1 . '-' . date('m-d H:i:s',$time));
			$eprice = 0 - $price;
			if(D('Users')->addGold($this->uid, $eprice ,'续费黄页包年关键词：'.$text)){
