...
class FarmAction extends CommonAction {
    ...
    public function index(){
        ...
        $count      = $fo->where($map)->count();// 查询满足要求的总记录数
    ...
    public function package(){ //套餐列表
        ...
        $count = $fp->where($map)->count(); // 查询满足要求的总记录数 
        $Page = new Page($count, 25); // 实例化分页类 传入总记录数和每页显示的记录数
        $show = $Page->show(); // 分页显示输出
    ...
    public function setpackage(){ //添加套餐
        $this->check_farm();
        if ($this->isPost()) {
