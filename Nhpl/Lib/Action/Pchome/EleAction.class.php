...
class EleAction extends CommonAction
{
    ...
    public function order()
    {
		...
		$settlement_price = (int) ($total['money'] - ($total['money']* $shop['rate'] / 1000));//先算出结算价，不按照后台为准
        ...
        if ($order_id = D('Eleorder')->add(array(
            ...
            foreach ($products as $val) {
                D('Eleorderproduct')->add(array('order_id' => $order_id, 'product_id' => $val['product_id'], 'num' => $val['buy_num'], 'total_price' => $val['price'] * $val['buy_num'], 'month' => $month));
            }
