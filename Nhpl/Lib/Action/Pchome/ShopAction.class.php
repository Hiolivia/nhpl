...
class ShopAction extends CommonAction
{
    ...
    public function dianping()
    {
        ...
        if (empty($shop_order) xor $tuan_order xor $ele_order) {
            $this->baoError('你还没在此商家消费过，暂时不能参与点评哦');
        }
        ...
        if (IS_AJAX) {
            ...
            if ($obj->add($data)) {
                ...
                if ($this->_CONFIG['sms']['dxapi'] == 'dy') {
                    D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_shop_yuyue_code', $data['mobile'], array('shop_name' => $detail['shop_name'], 'shop_tel' => $detail['mobile'], 'shop_addr' => $detail['addr'], 'code' => $data['code']));
                } else {
                ...
                if (!empty($detail['mobile'])) {
                    if ($this->_CONFIG['sms']['dxapi'] == 'dy') {
                        D('Sms')->DySms($this->_CONFIG['site']['sitename'], 'sms_shop_yuyue_shop', $detail['mobile'], array('name' => $data['name'], 'content' => $data['content'], 'yuyue_date' => $data['yuyue_date'], 'mobile' => $data['mobile'], 'number' => $data['number']));
                    } else {
                        D('Sms')->sendSms('sms_shangjia_yuyue', $detail['mobile'], array('name' => $data['name'], 'content' => $data['content'], 'mobile' => $data['mobile'], 'number' => $data['number'], 'yuyue_date' => $data['yuyue_date']));
                    }
                }
                //预约通知商家功能结束
    ...
    public function recognition()
    {
        ...
        if (IS_AJAX) {
            ...
            if ($obj->add($data)) {
                ...
                D('Email')->sendMail('pc_email_recognition', $pc_email_recognition, '你好，管理员：有客户认领商家了！', array('shop_name' => $shop_name, 'name' => $data['name'], 'mobile' => $data['mobile'], 'content' => $data['content']));
