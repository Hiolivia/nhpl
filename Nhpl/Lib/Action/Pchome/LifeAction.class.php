...
class LifeAction extends CommonAction
{
    ...
    public function index(){
        ...
        if ($keyword) {
            $map['qq|mobile|contact|title|num1|num2|addr'] = array('LIKE', '%' . $keyword . '%');//模糊查询蜂蜜二开
    ...
    public function create($cat){
        ...
        if ($this->isPost()) {
            ...
            $sms_open = $models->getField('sms_open');//后台管理员是否开通短信
            $sms_number = $models->getField('sms_number');//后台条目设定    
            //订阅发送短信 end
            ...
            if ($life_id = D('Life')->add($data)) {
                            ...
                            if ($v['sms_number']<$sms_number) {
                                ...
                                if ($v['sms']==1) {                                 
                                    ...
                                    if ($mobile) {
                                        ...
                                        $this->fengmiSuccess('手机发送成功！', U('member/life'));
                                    }
                ...
                $this->fengmiSuccess('发布信息成功，通过审核后将会显示！', U('member/life'));
            }
