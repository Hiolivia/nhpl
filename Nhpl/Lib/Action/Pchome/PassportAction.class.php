...
class PassportAction extends CommonAction
{
    ...
    public function wxlogin()
    {
        ...
        $url = $this->_CONFIG['site']['host'] . '/mobile/passport/weixincheck/state/' . $state . '.html';
        $this->assign('url', $url);
    ...
    private function thirdlogin($data)
    {
        if ($this->_CONFIG['connect']['debug']) {
            ...
            if (empty($connect['uid'])) {
                ...
                //p($face);die;
                $user = array('account' => $account, 'password' => rand(100000, 999999), 'nickname' => $nickname, 'ext0' => $account, 'face' => $face, 'create_time' => NOW_TIME, 'create_ip' => get_client_ip());
