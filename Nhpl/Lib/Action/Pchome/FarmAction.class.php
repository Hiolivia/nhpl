...
class FarmAction extends CommonAction {
    ...
    public function detail($farm_id){
        ...
        if(!$farm_id = (int)$farm_id){
            ...
            $fc = D('FarmComment'); // 实例化User对象
            import('ORG.Util.Page');// 导入分页类
            $count      = $fc->where('farm_id = '.$farm_id)->count();// 查询满足要求的总记录数
            ...
            $count2      = $fc->where(array('farm_id'=>$farm_id,'have_photo'=>1))->count();// 查询满足要求的总记录数
            $Page2       = new Page($count2,10);// 实例化分页类 传入总记录数和每页显示的记录数
            ...
            $this->assign('list2',$list2);// 赋值数据集www.hatudou.com  二开开发qq  120585022
