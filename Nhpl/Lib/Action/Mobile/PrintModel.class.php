...
class PrintModel{
    ...
    public function printOrder($msg,$shop_id) {
		...
		$params = array(
				...
				'machine_code'=>$shop['machine_code'],
				'time'=>$ti	
		);
		
		p($shop_id);die;
        $sign = $this->generateSign($params,$shop['apiKey'],$shop['mKey']);
		$params['sign'] = $sign;
		$params['content'] = $msg;
		...
		$data = rtrim($p, '&');
		return $this->liansuo_post($url,$data);
    }
