...
class PostAction extends CommonAction {
    ...
    public function index(){
        ...
        $map['p.closed'] = 0;
        $post = D('Post');
        $count = $post->alias('p')->where($map)->count();
        ...
        if($is_fine = (int)$this->_param('is_fine')){
            $map['p.is_fine'] = ($is_fine === 1 ? 1:0);
            $this->assign('is_fine',$is_fine);
        }
...
//        $list = $post->where($map)->order(array('post_id'=>'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
        ...
        $info = D('Users')->field('user_id,account,nickname')->where(['user_id'=>array('IN',$ids)])->select();
        foreach ($info as $row){
        ...
        $this->assign('cates', D('PostCate')->select());
    ...
    public function detail($post_id = 0){
        if ($post_id = (int) $post_id) {
           ...
           /* $detail['label_id'] = [1];
            $detail['cate_id'] = 1;
            $detail['title'] = '标题';
            $detail['details'] = '请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子请选择要查看的帖子';*/
//           $detail['pic'] = ['/attachs/2018/02/05/4bccff537c4db051be0f16ffd25b5f89.jpg'];

            $detail['pic'] = json_decode($detail['pic']);
            $detail['label_id'] = explode(',',$detail['label_id']);
            $detail['nickname'] = D('Users')->field('nickname')->find($detail['user_id'])['nickname'];

            $this->assign('detail', $detail);
            $this->assign('cates', D('PostCate')->select());
            $this->assign('labels', D('PostLabel')->select());
    ...
    public function fine($post_id = 0){
        if ($post_id){
            $obj = D('Post');
            $data = ['post_id'=>$post_id,'is_fine'=>1];
            if ($obj->save($data)){
                $this->baoSuccess('操作成功',U('post/index'));
            }
            $this->baoError('操作失败');
        }else{
            $this->baoError('请选择要操作的帖子！');
        }
    }
