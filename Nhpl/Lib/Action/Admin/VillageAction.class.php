...
class VillageAction extends CommonAction{
    private $create_fields = array('cate','name', 'addr', 'tel', 'pic', 'user_id', 'city_id', 'area_id', 'lng', 'lat','profiles', 'tribe_id','orderby', 'info', 'is_bbs');
    private $create_worker_fields = array('name', 'photo', 'village_id', 'job','orderby');
    private $edit_fields = array('cate','name', 'addr', 'tel', 'pic', 'user_id', 'city_id', 'area_id', 'lng', 'lat','tribe_id', 'orderby', 'info', 'profiles','is_bbs');
    private $look = 0;
	  ...
	  private function createCheck($iswork = 0){
        if ($iswork) {
			...
			if (!D('Village')->check_user_id_occupy($data['user_id'])) {
				$this->baoError('当前管理员已被其他乡村占用');
			}
			$data['profiles'] = htmlspecialchars($data['profiles']);
			$data['tribe_id'] = (int) $data['tribe_id'];
