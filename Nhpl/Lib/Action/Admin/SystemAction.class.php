...
class SystemAction extends CommonAction {
    ...
    private $create_fields = array('title', 'content', 'orderby','channel');
    private $edit_fields = array('title', 'content', 'orderby','channel');
    ...
    public function index() {
        ...
        $count = $Messages->where($map)->count(); // 查询满足要求的总记录数
        $Page = new Page($count, 25); // 实例化分页类 传入总记录数和每页显示的记录数
        $show = $Page->show(); // 分页显示输出
        $list = $Messages->where($map)->order(array('orderby' => 'asc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            $obj = D('Messages');
            if ($obj->add($data)) {
                $this->baoSuccess('添加成功', U('system/index'));
            }
            $this->baoError('操作失败！');
        } else {
            $this->display();
