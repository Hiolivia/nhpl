...
class HousekeepingcateAction extends CommonAction {
   ...
   private $create_fields = array('cate_name', 'orderby');
    ...
    public function select(){
        ...
        $count = $housekeepingcate->where($map)->count(); // 查询满足要求的总记录数 
    ...
    public function create($parent_id=0) {
        if ($this->isPost()) {
            ...
            if ($obj->add($data)) {
                $obj->cleanCache();
                $this->baoSuccess('添加成功', U('Housekeepingcate/index'));
            }
            $this->baoError('操作失败！');
        } else {
            $this->display();
        ...
        if ($cate_id = (int) $cate_id) {
            ...
            if ($this->isPost()) {
                ...
                if (false !== $obj->save($data)) {
                    $obj->cleanCache();
                    $this->baoSuccess('操作成功', U('Housekeepingcate/index'));
                }
                $this->baoError('操作失败');
            } else {
    ...
    public function delete($cate_id = 0) {
        if (is_numeric($cate_id) && ($cate_id = (int) $cate_id)) {
            ...
            $obj->delete($cate_id);
            $obj->cleanCache();
            $this->baoSuccess('删除成功！', U('Housekeepingcate/index'));
        } else {
    ...
    public function update() {
        ...
        $obj->cleanCache();
        $this->baoSuccess('更新成功', U('Housekeepingcate/index'));
    }
